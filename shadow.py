...
parser.add_argument('--endpoint',default="a5okrjfeg54i5-ats.iot.ap-south-1.amazonaws.com",  help="Your AWS IoT custom endpoint, not including a port. " +
                                                      "Ex: \"w6zbse3vjd5b4p-ats.iot.us-west-2.amazonaws.com\"")
parser.add_argument('--cert',default="/home/avinash/catkin_ws/src/mqtt_ros_aws_iot/config/testCerts/8e33153ddf0234e47f9fcc40c676f8f436f66ecea69ba2ceb3c39b44d000e9a1-certificate.pem.crt",  help="File path to your client certificate, in PEM format")
parser.add_argument('--key',default="/home/avinash/catkin_ws/src/mqtt_ros_aws_iot/config/testCerts/8e33153ddf0234e47f9fcc40c676f8f436f66ecea69ba2ceb3c39b44d000e9a1-private.pem.key", help="File path to your private key file, in PEM format")
parser.add_argument('--root-ca',default="/home/avinash/catkin_ws/src/mqtt_ros_aws_iot/config/testCerts/AmazonRootCA1.pem", help="File path to root certificate authority, in PEM format. " +
                                      "Necessary if MQTT server uses a certificate that's not already in " +
                                      "your trust store")
parser.add_argument('--client-id', default="test-" + str(uuid4()), help="Client ID for MQTT connection.")
parser.add_argument('--thing-name',default="ubuntuPC",
  help="The name assigned to your IoT Thing")
parser.add_argument('--shadow-property', default="blinker", help="Name of property in shadow to keep in sync")
parser.add_argument('--use-websocket', default=False, action='store_true',
    help="To use a websocket instead of raw mqtt. If you " +
    "specify this option you must specify a region for signing.")
parser.add_argument('--signing-region', default='ap-south-1', help="If you specify --use-web-socket, this " +
...
thing_name = ""
shadow_property = ""
SHADOW_PROPERTY_DEFAULT = "blinker"
...
def on_get_shadow_accepted(response):
    ...
    try:
        ...
        if response.state:
            if response.state.delta:
                ...
                if value:
                    ...
                    change_shadow_value(value,shadow_property)
                    return
        ...
        change_shadow_value(SHADOW_VALUE_DEFAULT,SHADOW_PROPERTY_DEFAULT )
        return
...
def on_update_shadow_accepted(response):
    ...
    try:
        ...
        try:
            print("Finished updating reported shadow value of "+ shadow_property +" to '{}'.".format(response.state.reported[shadow_property])) # type: ignore
            print (response.state.reported[shadow_property])
            
            if shadow_property == "blinker" and (response.state.reported[shadow_property] == "ON"):
                print ("LED ON")
                os.system('python test.py 1')

            if shadow_property == "buzzer" and (response.state.reported[shadow_property] == "ON"):
                print ("BUZZER ON")
                os.system('python test.py 2')


            print("Enter desired value(shadow_value) to be changed: ") # remind user they can input new values
        except:
...
def change_shadow_value(value,shadow_property_value):
    global shadow_property
    # print (shadow_property)
    # print(value,shadow_property_value)
    with locked_data.lock:
        ...
        if locked_data.shadow_property == shadow_property_value:
            print("The same shadow property '{}' is chosen".format(value))
        else:
            print("The new shadow prperty '{}' is chosen".format(shadow_property_value))
            locked_data.shadow_property = shadow_property_value
            shadow_property = shadow_property_value
            # print(shadow_property)
...
def user_input_thread_fn():
    while True:
        try:
            # Read user input
            new_value = input()
            print("Enter the desired shadow property")
            shadow_property_new_value = input()
            print(shadow_property_new_value + " " + new_value)
            

            # If user wants to quit sample, then quit.
            # Otherwise change the shadow value.
            if (new_value,shadow_property_new_value) in ['exit', 'quit']:
                exit("User has quit")
                break
            else:
                change_shadow_value(new_value,shadow_property_new_value)
